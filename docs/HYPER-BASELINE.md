...
| `hypercurve` | `08fb7fef66720b123d32cf94d3e0528eea1c83fd` | concurrent tracked edits in `src/bezier_offset.rs`, `src/bezier_region.rs`, and `src/curve.rs`; qualified observed diff SHA-256 `cd562aeada7607c31b290db51bc81025fd056cbe56d75bad443283c7328941d8` |
| `hyperpath` | `c65e0136514637305e99e3eadd5432ef5e234e68` | clean; includes exact diagonal feed-segment length plus exact-stop lookahead support used here |
...
The certified cubic-motion checkpoint's final native/WASM tests, strict
checks, optimized bundle, decompression checks, and loopback render observed
Hypercurve at HEAD `08fb7fef66720b123d32cf94d3e0528eea1c83fd` with the tracked diff recorded in
the table. Its optimized dependency build also reported the concurrently
introduced unused private helper
`retained_selected_fillet_parameter_is_in_open_range`; Alumina did not edit or
suppress that warning. Hypercurve advanced through multiple coherent and
temporarily non-compiling states while this work was underway and is expected
to continue changing. This is an observed/tested development state, not a
request to hold, reset, or pin that working tree. The Hyperphysics tracked diff
remained SHA-256
